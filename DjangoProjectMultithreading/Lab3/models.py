...
class TrainRoute(models.Model):
    ...
    FullConnectionName = models.CharField(db_column='FullConnectionName', max_length=50, blank=True, null=True)
...
class TrainRouteWithSquadOnDate(models.Model):
    ...
    def __str__(self):
        full_train_id = str(self.FullTrainIdentificator)
        year = int(full_train_id[-2:])
        month = int(full_train_id[-4:-2])
        day = int(full_train_id[-6:-4])
        train_number = int(full_train_id[:-6])
        return f"Склад поїзда номер {train_number} в дату {day}.{month}.{year}"
...
class TrainSquad(models.Model):
    ...
    class Meta:
        db_table = 'TrainSquad'
    def __str__(self):
        train_part_str = str(self.TrainSquadID);
        last_digit = train_part_str[-1]
        train_number_code = ""
        for digit in train_part_str:
            if digit == '0':
                break
            train_number_code += digit

        return f"{last_digit}-й склад поїзда {train_number_code}"
