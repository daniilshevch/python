...
from django.shortcuts import render
from analytics.multithreading_visualization import generate_dashboard_graph
from bokeh.plotting import figure
from bokeh.models import ColumnDataSource
@api_view(['GET'])
...
def dashboard_view(request):
    ...
    graph_html, table_html = generate_dashboard_graph(refresh=refresh)
    return render(request, 'analytics/multithreading_dashboard.html', {
        'graph': graph_html,
        'table': table_html
    })

def refresh_dashboard_data(request):
    graph_html, table_html = generate_dashboard_graph(refresh=True)
    return JsonResponse({
        'graph': graph_html,
        'table': table_html
    })